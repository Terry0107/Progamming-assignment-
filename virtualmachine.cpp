...
// Flags - they update based on result of operations
 ...
 void updateFlags(int result, int operan1 = 0, int operand2 = 0, string op = "" ) {
    ZF = (result == 0 ); // result zero?
    OF = (result > 127 || result < -128); // overflow check
    UF = (result < -128 ); // underflow check
    if (op == "ADD")CF = ((unsigned char )operand1+ (unsigned char )operand2 > 255);
    else if (op == "SUB") CF = ((unsigned char ) operand1 < (unsigned char ) operand2);
 ...
 void SUB(char registers[], int src, int dst){
   ...
   registers[dst] = (char)result;
   cout << (int)registers[dst] << endl; 
   updateFlags(result, register[dst], registers[src], "SUB");
 }
